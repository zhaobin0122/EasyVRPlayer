...
public class Pvr_ControllerModuleInit : MonoBehaviour
{
    ...
    public void UpdateRay()
    {
        ...
        if (isupdate && rayLine != null && rayLine.gameObject.activeSelf)
        {
            ...
            if (type == 1)
            {
                rayLine.GetComponent<LineRenderer>().SetPosition(0, transform.TransformPoint(0, 0, 0.08f));
            }
            else
            {
                rayLine.GetComponent<LineRenderer>().SetPosition(0, transform.TransformPoint(0, 0.009f, 0.08f));
            }
