...
public unsafe class InkeMediaPlayer : MonoBehaviour {
#else
public class InkeMediaPlayer : MonoBehaviour
{
#endif
	...
	public void Resize()
	{
		...
		if (m_objResize != null)
		{
			...
			for (int i = 0; i < m_objResize.Length; i++)
			{
				...
				m_objResize[i].GetComponent<RectTransform>().sizeDelta = new Vector2(800, 800);
    ...
    void ChangeStreamUrl(string newStreamUrl)
	{
		GetJavaObject().Call("NDK_SetFileName", newStreamUrl);
        GetJavaObject().Call("NDK_SetPlayType", (int)LiveRoomDataHolder.streamType, (int)LiveRoomDataHolder.playType);
		Call_RePlay(0);
	}
	...
	if( SystemInfo.graphicsMultiThreaded == true)
	{
	GetJavaObject().Call("NDK_SetFileName", LiveRoomDataHolder.streamUrl);
    GetJavaObject().Call("NDK_SetPlayType", (int)LiveRoomDataHolder.streamType, (int)LiveRoomDataHolder.playType);
	...
	public void Call_RePlay(int position)
	{
	GetJavaObject().Call("RePlay", position);
	}
    ...
    public void Call_Pause2()
	{
      GetJavaObject().Call("Pause2");  
    } 

    public void Call_Resume()
    {
      GetJavaObject().Call("Resume");
    }
